...
export default function EmployerDashboard() {
  ...
  const handleInterest = async (certificateId, value) => {
    // Toggle off if clicking the active marker again
    const current = history.find((h) => h.certificateId === certificateId)?.interest;
    const next = current === value ? null : value;
    const updated = await persistInterest(certificateId, next);
    setHistory(updated);
  };
          ...
          <div className="space-y-4 print:hidden">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-slate-700">Saved candidates</h2>
                {history.length > 0 && (
              ...
              {history.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-slate-400">
                    Verified carers are saved here (last 5). Mark them interested to follow up.
                  </p>
                </div>
              ) : (
                  ...
                  {history.map((item) => {
                    ...
                    return (
                      <li key={item.certificateId}
                        ...
                        <button onClick={() => handleHistoryClick(item)} className="w-full text-left">
                        ...
                        <div className="flex items-center gap-1.5 mt-2.5">
                          <button onClick={() => handleInterest(item.certificateId, "yes")}
                            className={`flex-1 inline-flex items-center justify-center gap-1 px-2 py-1.5 rounded-lg text-xs font-medium transition ${
                              item.interest === "yes"
                                ? "bg-emerald-600 text-white"
                                : "bg-emerald-50 text-emerald-700 hover:bg-emerald-100"
                            }`}>
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M6.633 10.5c.806 0 1.533-.446 2.031-1.08a9.041 9.041 0 012.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 00.322-1.672V3a.75.75 0 01.75-.75A2.25 2.25 0 0116.5 4.5c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282h3.126c1.026 0 1.945.694 2.054 1.715.045.422.068.85.068 1.285a11.95 11.95 0 01-2.649 7.521c-.388.482-.987.729-1.605.729H13.48c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 00-1.423-.23H5.904M14 9h-2.25M9 18.75h.008" />
                            </svg>
                            Interested
                          </button>
                          <button onClick={() => handleInterest(item.certificateId, "no")}
                            className={`flex-1 inline-flex items-center justify-center gap-1 px-2 py-1.5 rounded-lg text-xs font-medium transition ${
                              item.interest === "no"
                          ...
                          <button onClick={() => handleRemove(item.certificateId)}
                            className="px-2 py-1.5 rounded-lg text-slate-300 hover:text-red-500 hover:bg-red-50 transition"
                            aria-label="Remove">
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                            </svg>
